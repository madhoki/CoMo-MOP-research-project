...
# folder = 'twa_mop_cavity_data'
# output_csv = 'all_results_for_twa.csv'
...
with open(output_csv, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['filename', 'avg_metal_dist', 'inner_atom', 'inner_diameter', 'inner_volume', 'max_pore_size_diameter', 'max_pore_size_volume'])
    count = 0
        ...
        if filename.endswith('.xyz'):
            ...
            try:
                ...
                if filename == selected_xyz_file:
                    max_pore_size, max_pore_vol, best_vec_selected = max_pore_size_all_directions(atoms, coords, n_directions=1000)
                    coords_selected, atoms_selected = coords, atoms
                else:
                    max_pore_size, max_pore_vol, _ = max_pore_size_all_directions(atoms, coords, n_directions=1000)
                writer.writerow([filename, avg_metal, inner_atom, round(inner_diam,2), round(inner_vol,2), round(max_pore_size,2), round(max_pore_vol,2)])
                count += 1
